/*
    ...
    For more information, please visit http://www.oasys.co/oasysx
      ...
      DATE:           02/01/2020
*/
#ifndef __imag_conf_h
#define __imag_conf_h


#include <stdio.h>
#include "fill_pixe.h"
#include "compare_minutia.h"
#include <bitset>
...
namespace minutia_fetch{
  ...
  struct maximum5_fast {
    ...
    void max_num_mem(size_t i, size_t j) {
      size_t mi = i;
      size_t mj = j;
  ...
  struct SmmeParameters {
    ...
    int32 Tb;
    minutia_capture::pixel_subscribe<PixelG>::ptr_t diagG;
    minutia_capture::pixel_subscribe<bool>::ptr_t   diagMC;
    minutia_capture::pixel_subscribe<uint8>::ptr_t  diagOri;
  ...
  inline bool adjust_angle(uint8 &a, size_t xp, size_t yp, const uint8 * p, size_t width, size_t size, bool relative) {
    ...
    if (d0 >= 14 * 14) {
      locate base = cur - p2;
      a = minutia_extractor::atan2(-base.y, base.x);
    } else {
      return false;
    }
    return true;
  ...
  void extract_minutia(const uint8 * phasemap, size_t width, size_t size, const uint8 * footprint, top_n<Minutia>& top_minutia, const Parameters & param) {
    ...
    convert_33<stride,2,1,5> cxx(width), cxy(width), cyy(width);
    typedef maximum5_fast<int32, stride> max5_t;
    max5_t max5;
    delay<bool, stride * (orifilt_size - max5_t::yoffs) - max5_t::xoffs> delay_mc(width * (orifilt_size - max5.yoffs) - max5.xoffs);
    fill_pixe bf(width, size, phasemap, param.fill_pixe_);
    ...
    for (size_t y = 0; y < height + orifilt_size; y++) {
      for (size_t x = 0; x < width; p++, x++) {
        ...
        if (mc) {
          ...
          if (confirmed) {
            // Minutia
            Minutia m;
            m.position.x = int16(xp); // + minutia_extractor::reduce<int16>(bf.xoffs, 7);
            m.position.y = int16(yp); // + minutia_extractor::reduce<int16>(bf.yoffs, 7);
            if (bf.rotate180) {
              a = (a + 128) & 0xff;
            }
            ...
            m.type = confidence > param.fill_pixe_.type_thresold ? type ? Minutia::type_ridge_ending : Minutia::type_bifurcation : Minutia::type_other; //bf.confidence 
            top_minutia.add(m);
          }
        }
      }
      if ((y & 1)==0) {
        ...
        for (size_t x = 0; x < width/2 + orifilt_size/2; x++) {
          ...
          if (x >= orifilt_size/2) {
            uint8 a = minutia_extractor::atan2(s2.real(), s2.imag()) / 2;
